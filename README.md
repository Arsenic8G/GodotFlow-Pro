...
[![Godot](https://img.shields.io/badge/Godot-4.x-blue?style=for-the-badge&logo=godotengine)](https://godotengine.org/)
...
- ⚡ **Smart Godot 4 Collision Pipeline:**
  - Generates Convex (`-convcolonly`) and Trimesh (`-colonly`) shapes on the fly.
  - Non-destructive **Decimate** modifier support for collision optimization.
- 🎭 **Animation Library Splitter:** Exports animations as modular `Character@Walk-loop.glb` files and patches `.import` files for Godot 4 `AnimationLibrary`.
- 📂 **Persistent Project Manager:** Easily switch between different Godot project root paths (`res://`).
- 🧹 **Clean Skeleton Export:** Filters out IK helper bones and control shapes, keeping only deformation bones.
- 🔄 **Async Update Checker:** Non-blocking background updater that alerts you to new releases directly in the 3D Viewport.
...
5. Press `N` in the 3D Viewport and open the **Godot** tab.
